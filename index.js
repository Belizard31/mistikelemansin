...
Marketten kazma satın alırsınız.`,
      ...
      await interaction.reply({
        content: `Bu butonu sadece <@${userId}> kullanabilir!`,
        ephemeral: true,
      });
    }
  }
...
🪙 **</para çek:0>**
Oluşturduğunuz kartın banka hesabından para çekersiniz.`,
          inline: true,
        })
        .setFooter({
          text: `${user.username} tarafından istendi.`,
...
👑 **</para-sil:0>**
Etiketlediğiniz kullanıcıdan yazdığınız miktar kadar para siler.`,
          inline: true,
        })
        .setTimestamp()
        .setFooter({
...
client.on("interactionCreate", async (interaction) => {
  if (interaction.customId === "mzrkartolustur") {
    ...
    if (kartNumarası) {
      return interaction.reply({
        content: "> Mevcut bir kredi kartın bulunuyor!",
        ephemeral: true,
      });
    }
    ...
    const kartCVC = Math.floor(Math.random() * 1000)
      .toString()
      .padStart(4, "0");
    ...
    mzrdb.set(`mzrkart.${user.id}`, {
      kartNumara: kartNumara,
      cvc: kartCVC,
      sonKullanım: kartSonKullanım,
    });
  }
  if (interaction.customId === "mzrkartsil") {
    ...
    if (!kartNumara) {
      return interaction.editReply({
        content: "> Cüzlanında bu kadar para yok!",
      });
    }
    ...
    const mzrEmbed = new EmbedBuilder()
      ...
      .setDescription(
        `Aşağıdaki kart bilgili kartınızın **silinmesini** onaylıyormusunuz?`,
      )
      .setColor("Red")
      .addFields(
        {
          name: "💳 Kart Numarası",
          value: `${codeBlock("yaml", `${kartNumara}`)}`,
          inline: true,
        },
        {
          name: "💳 Kartın CVC",
          value: `${codeBlock("yaml", `${kartCVC}`)}`,
          inline: true,
        },
        {
          name: "💳 Kart Son Kullanım Tarihi",
          value: `${codeBlock("yaml", `${kartSonKullanım}`)}`,
          inline: true,
        },
        {
          name: "💸 İçindeki Para",
          value: `${codeBlock("yaml", `${içindeki} TL`)}`,
          inline: true,
        },
      )
      .setTimestamp()
  ...
  if (interaction.customId === "mzrkartsilonay") {
    ...
    if (!kartNumara) {
      return interaction.editReply({
        content:
          "> Mevcut bir kartın bulunmuyor! Kart oluşturmak için: **/kart oluştur**",
      });
    }
    ...
    const mzrEmbed = new EmbedBuilder()
      ...
      .setDescription(
        `Kartını başarıyla sildim! aşağıdaki bölümden detayları görebilirsin.`,
      )
      .addFields(
        {
          name: "💳 Kart Numarası",
        ...
        {
          name: "💸 Silinen Para",
          value: `${codeBlock("yaml", `${silinen} TL`)}`,
          inline: true,
        },
      )
      .setColor("Green")
